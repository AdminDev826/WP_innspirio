<?php
/*
Plugin Name:  Pageperf Autoptimize Fork
Plugin URI: http://pageperf.com
Description: Pageperf fork of Autoptimize plugin 
Version: 2.0.0
...
function autoptimize_end_buffering($content) {
  ...
  if ( has_filter('autoptimize_override_urls') )
  {
    $urls = apply_filters('autoptimize_override_urls', '');

    if ( is_array( $urls ) )
    {
      define('AUTOPTIMIZE_WP_SITE_URL', 
        $urls[ 'AUTOPTIMIZE_WP_SITE_URL' ]);

      define('AUTOPTIMIZE_WP_CONTENT_URL', 
        $urls[ 'AUTOPTIMIZE_WP_CONTENT_URL' ]);
    }
  }
